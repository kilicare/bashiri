export function MilestoneCard({ data }: { data: any }) {
  return (
    <div className="rounded-3xl p-5 text-center" style={{ background: "rgba(245,166,35,0.06)", border: "1px solid rgba(245,166,35,0.2)" }}>
      <div className="flex items-center justify-center gap-2 mb-2">
        {data.avatar_url ? (
          <img src={data.avatar_url} alt={data.username} className="w-8 h-8 rounded-xl object-cover" />
        ) : (
          <div className="w-8 h-8 rounded-xl flex items-center justify-center font-black text-black" style={{ background: "#F5A623" }}>
