"use client";
import { usePathname, useRouter } from "next/navigation";
import { motion } from "framer-motion";
import { Home, Calendar, Sparkles, User, Plus } from "lucide-react";

const LEFT_ITEMS = [
  { href: "/home", icon: Home, label: "Home" },
  { href: "/matches", icon: Calendar, label: "Matches" },
];
const RIGHT_ITEMS = [
  { href: "/ai", icon: Sparkles, label: "AI" },
  { href: "/profile", icon: User, label: "Profile" },
];

function NavButton({ item, active }: { item: any; active: boolean }) {
  const router = useRouter();
  const Icon = item.icon;
  return (
    <button onClick={() => router.push(item.href)} className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl flex-1">
      <Icon
        size={20}
        style={{
          color: active ? "#FBBF24" : "rgba(255,255,255,0.4)",
          filter: active ? "drop-shadow(0 0 6px rgba(251,191,36,0.6))" : "none",
        }}
      />
      <span className="text-[10px] font-bold" style={{ color: active ? "#FBBF24" : "rgba(255,255,255,0.35)" }}>
        {item.label}
      </span>
    </button>
  );
}

export function BottomNav() {
  const pathname = usePathname();
  const router = useRouter();

  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-30 pb-safe"
      style={{ background: "rgba(13,7,20,0.95)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(124,58,237,0.2)" }}
    >
      <div className="flex items-center justify-around px-2 py-2 relative max-w-lg mx-auto">
        {LEFT_ITEMS.map((item) => (
          <NavButton key={item.href} item={item} active={pathname.startsWith(item.href)} />
        ))}

        <motion.button
          onClick={() => router.push("/create")}
          className="w-14 h-14 rounded-2xl flex items-center justify-center -mt-6 mx-2 shrink-0"
          style={{ background: "#FBBF24", boxShadow: "0 8px 25px rgba(251,191,36,0.5)" }}
          whileTap={{ scale: 0.9 }}
        >
          <Plus size={26} strokeWidth={2.8} style={{ color: "#1A1000" }} />
        </motion.button>

        {RIGHT_ITEMS.map((item) => (
          <NavButton key={item.href} item={item} active={pathname.startsWith(item.href)} />
        ))}
      </div>
    </nav>
  );
}