...
regular_user = User.objects.create_user(phone_number='+255111111111', password='RegularPass123!')
request = factory.post(f'/api/dashboard/users/{user.id}/reset-password/',
    json.dumps({'new_password': 'HackedPass123!'}),
    content_type='application/json'
)
force_authenticate(request, user=regular_user)
response4 = view(request, user.id)
print(f'Test 4 - Non-admin user attempting reset:')
print(f'  Status: {response4.status_code} (expected 403)')
